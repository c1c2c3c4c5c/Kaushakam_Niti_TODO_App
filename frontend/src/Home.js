...
const Home = () => {
    ...
    useEffect(() => {
        axios.get('https://kaushalam-niti-todo-app.onrender.com/get')
            .then(result => setTodos(result.data))
            .catch(err => console.log(err));
    }, []);

    const edit = (id) => {
        axios.put(`https://kaushalam-niti-todo-app.onrender.com/edit/${id}`)
            .then(result => {
    ...
    const Update = (id, updatedTask) => {
        axios.put(`https://kaushalam-niti-todo-app.onrender.com/update/${id}`, { task: updatedTask })
            .then(result => {
                console.log(result.data);
                const updatedTodos = todos.map(todo => {
                    if (todo._id === id) {
    ...
    const Hdelete = (id) => {
        axios.delete(`https://kaushalam-niti-todo-app.onrender.com/delete/${id}`)
            .then(result => {
                console.log(result.data);
                        ...
                        <div className='task' key={todo._id}>
                            ...
                            <div className='checkbox'>
                                ...
                                {taskid === todo._id ?
                                    ...
                                    :
                                    <p id="tap">Tap the circle icon to list out the completed task!!
                                        <p className={todo.done ? 'through' : 'normal'}>{todo.task}</p></p>
                                }
                            </div>
                            <div>
                                <span>
